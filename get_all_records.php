...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $entryDate = htmlspecialchars(formatDateForMask($row['entry_date'] ?? ''), ENT_QUOTES, 'UTF-8');
        $outDate = htmlspecialchars(formatDateForMask($row['out_date'] ?? ''), ENT_QUOTES, 'UTF-8');
        ...
        echo "<td class='table-cell'>
                <div style='display: flex; flex-direction: column; gap: 10px;'>
                    ...
                    " . (canDelete() ? "<button class='delete-btn table-btn' data-id='" . $rowId . "'>Удалить</button>" : "") . "
                </div>
              </td>";
        echo "</tr>";
    }
